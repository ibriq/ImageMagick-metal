  ...
  const MagickBooleanType sharpen, ExceptionInfo *exception)
{
...

  {
    ...
    args[5] = MetalBytesArg(&rows, sizeof(rows));
...
MagickPrivate MagickBooleanType AccelerateBlurImageMetal(Image *image,
  const double radius, const double sigma, ExceptionInfo *exception)
{
  ...
  imageBuffer = AcquirePixelCacheBuffer(device, image, &is_nocopy,
    exception);
  if (imageBuffer == NULL)
    return MagickFalse;

  {
    ...
    GetPixelCachePixels(image, &cache_length, exception);
  ...
  const PixelIntensityMethod method, ExceptionInfo *exception)
{
...

  {
    ...
    args[6] = MetalBytesArg(&rows, sizeof(rows));
  ...
  const double *parameters, ExceptionInfo *exception)
{
...

  {
    ...
    args[7] = MetalBytesArg(&quantum_scale, sizeof(quantum_scale));
  ...
  ExceptionInfo *exception)
{
...

  {
    ...
    status = EnqueueMetalKernel2DEx(device, kernel, global, local, 0, args, 6);
  }
